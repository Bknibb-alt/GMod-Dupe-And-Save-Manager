...
root = tk.Tk()
root.title("Garry's Mod Dupe Manager")
...
class dupe(ttk.Frame):
    ...
    def change_image(self):
        ...
        refresh()
class renamer(tk.Tk):
    def __init__(self, dupe_):
        super().__init__()
        self.dupe = dupe_
        self.title("Rename Dupe")
        self.name_var = tk.StringVar(master=self, value=self.dupe.name)
...
def refresh():
    ...
    for i in os.listdir(dupes_path):
        if os.path.splitext(i)[-1] == ".dupe":
            dupes += [dupe("".join(os.path.splitext(i)[:-1]), frame.interior)]
            dupes[-1].grid(row=y, column=x, sticky="nesw")
            x += 1
            if x >= width:
                x = 0
                y += 1
...
menu.add_command(label="Delete", command=lambda: selected.delete())
menu.add_command(label="Rename", command=lambda: selected.rename())
menu.add_command(label="Duplicate", command=lambda: selected.duplicate())
menu.add_command(label="Change Image", command=lambda: selected.change_image())
menu.add_separator()
menu.add_command(label="Refresh All", command=refresh)
